...
impl<'a> TokenStream<'a> {
    ...
    pub fn scan(&mut self) -> Result<Token, Unwind> {
        ...
        if self.at_special() {
            ...
            let start = self.next();
            println!("=> special");
            return self.result(Token::SPECIAL, start..self.pos());
        }
        //
        // 4. If at a digit character, consume it. Then continue as below,
        //    returning the appropriate type of number token.
        //
        //    NOTE: It is a syntax error if a number parsed according
        //    to following rules is followed by a word character.

        if self.at_digit(10) {
            println!("scan 4: number");
            let n = self.scan_number();
            if self.at_word() {
                return self.error("Number followed by a word character");
            } else {
                return n;
            }
        }
        ...
        if self.at_word() {
            ...
            if self.at_char(':') {
                ...
                if self.at_char(':') {
                    self.reset(pos);
                    println!(
                        "scan 9: second colon find, reset to {:?} using {}",
                        self.current, pos
                    );
                } else {
                    println!("=> keyword");
    ...
    fn scan_number(&mut self) -> Result<Token, Unwind> {
        ...
        if self.at_char('x') || self.at_char('X') {
            self.next();
            while self.at_digit(16) || self.at_char('_') {
                self.next();
            }
            println!("=> hex number");
        ...
        //
        // 4.2. If at b or B, consume binary digits and return BINARY_INTEGER.
        //
        if self.at_char('b') || self.at_char('B') {
            self.next();
            while self.at_digit(2) || self.at_char('_') {
                self.next();
            }
